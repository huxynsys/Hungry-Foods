...
$stats['today_revenue'] = $conn->query("SELECT COALESCE(SUM(total),0) FROM orders WHERE DATE(created_at) = CURDATE()")->fetchColumn();
...
// Recent Orders
$orders = $conn->query("SELECT o.*, u.full_name, u.email FROM orders o LEFT JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC LIMIT 50")->fetchAll();
...
$monthly = $conn->query("SELECT DATE_FORMAT(created_at,'%b') as month, COALESCE(SUM(total),0) as revenue FROM orders WHERE created_at >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) GROUP BY YEAR(created_at),MONTH(created_at),DATE_FORMAT(created_at,'%b') ORDER BY MAX(created_at) ASC")->fetchAll();

// Top selling items
$top_items = $conn->query("SELECT oi.item_name AS menu_item_name, SUM(oi.quantity) as total_qty, SUM(oi.quantity * oi.price) as total_rev FROM order_items oi GROUP BY oi.item_name ORDER BY total_qty DESC LIMIT 5")->fetchAll();

// Customers
$customers = $conn->query("SELECT id, full_name, email, created_at, (SELECT COUNT(*) FROM orders WHERE user_id = s.id) as orders_count FROM users s ORDER BY created_at DESC LIMIT 50")->fetchAll();
?>
<!DOCTYPE html>
<html lang="en">
<head>
